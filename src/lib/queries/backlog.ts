...
export async function getBacklogData(db: SupabaseClient, warehouseCode: string) {
  ...
  const alerts = orderIds.length
    ...
    : ([] as { order_id: string; time_alert: string | null; elapsed_minutes: number; is_picking_backlog: boolean; is_verification_backlog: boolean }[])
